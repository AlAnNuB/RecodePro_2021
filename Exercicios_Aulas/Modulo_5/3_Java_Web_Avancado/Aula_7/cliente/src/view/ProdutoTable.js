...
export class ProdutoTable extends React.Component {
    ...
    execAltera = (codigo, nome, quantidade, descricao, ativo, cadastro) => {
        this.setState({formVisivel:false});
        alterarProduto(codigo,nome,quantidade,descricao,ativo, cadastro).then(this.props.execRefresh());
    }
    ...
    render() {
        ...
        <>
            ...
            {this.props.items.map(item => (
                <tr key={item.id}>
                <th>{item.id}</th>
                <th><span className={item.ativo?"":"text-danger"}>{item.nome}</span></th>
